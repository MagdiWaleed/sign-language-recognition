...
class GlossesRecognition():
    ...
    def start_recognition(self):
        reading = threading.Thread(target=self.readFrames)
        showing = threading.Thread(target=self.showFrames,daemon=True)
        predict_glosses = threading.Thread(target=self.getGlossPredictions)
    ...
    def readFrames(self): 
        ...
        while self.reading_counter<1000:
            ret, frame = cap.read()
            if not ret:
            ...
            if self.reading_counter%self.length_of_frames_predictions == 0:
                self.total_glosses_counter+=1

    def showFrames(self):
        while True: 

            if self.reading_counter ==0 and self.showing_counter ==0:
                frame = np.zeros((224, 224, 3), dtype=np.uint8)
            elif self.showing_counter < self.reading_counter:
                frame = self.totalframes[self.showing_counter-1]
            elif self.reading_counter == 0:
                frame = np.zeros((224, 224, 3), dtype=np.uint8)
            else:
                frame = self.totalframes[self.reading_counter-1]

            cv2.putText(
                    frame,"sh-"+str(self.showing_counter+1 )+", rd-"+str(self.reading_counter+1),
    ...
    def getGlossPredictions(self):
        while True:
            if self.current_glosses_counter < self.total_glosses_counter:
                ...
                self.current_glosses_counter += 1

                print(f"Predicted gloss: {gloss_string}, showing_c: {self.showing_counter}, reading_c: {self.reading_counter}")
